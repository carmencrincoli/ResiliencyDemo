...
var combinedSetupCommand = 'echo "=== Phase 1: Installing Azure CLI ===" && curl -sL https://aka.ms/InstallAzureCLIDeb | bash && echo "=== Phase 2: Authenticating with Managed Identity ===" && az login --identity --allow-no-subscriptions && echo "=== Phase 3: Downloading Setup Script ===" && az storage blob download --account-name ${storageAccountName} --container-name assets --name deployscripts/webapp-setup.sh --file /tmp/webapp-setup.sh --auth-mode login && chmod +x /tmp/webapp-setup.sh && sed -i "s/\r$//" /tmp/webapp-setup.sh && echo "=== Phase 4: Setting up Web Application ===" && ${envExports} && /bin/bash /tmp/webapp-setup.sh && echo "=== All phases completed successfully ==="'
