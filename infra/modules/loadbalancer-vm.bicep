...
var combinedSetupCommand = 'echo "=== Phase 1: Installing Azure CLI ===" && curl -sL https://aka.ms/InstallAzureCLIDeb | bash && echo "=== Phase 2: Authenticating with Managed Identity ===" && az login --identity --allow-no-subscriptions && echo "=== Phase 3: Installing Bash ===" && az storage blob download --account-name ${storageAccountName} --container-name assets --name deployscripts/bash-installer.sh --file /tmp/bash-installer.sh --auth-mode login && chmod +x /tmp/bash-installer.sh && sed -i "s/\r$//" /tmp/bash-installer.sh && /bin/sh /tmp/bash-installer.sh && echo "=== Phase 4: Downloading Setup Script ===" && az storage blob download --account-name ${storageAccountName} --container-name assets --name deployscripts/loadbalancer-setup.sh --file /tmp/loadbalancer-setup.sh --auth-mode login && chmod +x /tmp/loadbalancer-setup.sh && echo "=== Phase 5: Setting up Load Balancer ===" && ${envExports} && /bin/bash /tmp/loadbalancer-setup.sh && echo "=== All phases completed successfully ==="'
